...
function isPalindrome(str) {
  const regex = /[,?! ]/g;
  const strReversed = reverseString(str).toUpperCase().replaceAll(regex, '');
  return strReversed === str.toUpperCase().replaceAll(regex, '');
}
...
function reverseWords(str) {
  ...
  str.split(' ').forEach((element) => {
    arrs.push(reverseString(element));
  });
  return arrs.join(' ');
}
...
function encodeToRot13(str) {
  ...
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    if (encoded.includes(str[i])) {
      res += encoded[alpha.indexOf(str[i])];
    } else {
      res += str[i];
    }
  }
